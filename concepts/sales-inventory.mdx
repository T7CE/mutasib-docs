...
description: "Record sales, auto-decrement stock, generate shareable digital receipts, track inventory movements, adjust stock manually, and manage customer credit notes — all in one place."
---
...
Retrieve all credit notes for your shop. You can filter by status to see only open, paid, or overdue notes:

```bash
curl "https://api.mutasib.com/api/v1/credit-notes/{shop_id}/?status=open" \
  -H "Authorization: Bearer <token>"
```

### Updating a Credit Note

Mark a note as paid or update its details:

```bash
curl -X PATCH https://api.mutasib.com/api/v1/credit-notes/{shop_id}/{note_id}/ \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{ "status": "paid" }'
```
...
Customers can retrieve their own credit notes across all shops without needing shop-owner credentials:

```bash
curl https://api.mutasib.com/api/v1/credit-notes/my/ \
...
| `GET` | `/api/v1/credit-notes/{shop_id}/` | List credit notes for a shop |
| `PATCH` | `/api/v1/credit-notes/{shop_id}/{note_id}/` | Update a credit note |
| `GET` | `/api/v1/credit-notes/my/` | Customer's own credit notes |
