...
Mutasib secures every API request with a long-lived **API token**. Generate a token from your [Mutasib dashboard](https://mutasib.com/dashboard) and attach it to every request as an `Authorization: Bearer` header. All programmatic access to the API uses API tokens.
...
3. Create a new token and copy it immediately. Tokens are shown only once at creation time.
...
| --- | --- | --- | --- |
| `401` | Unauthorized | No token was provided, the token is invalid, or the token has been revoked. | Include a valid `Authorization: Bearer` header with an active API token. Generate a new token from the dashboard if needed. |
