...
<Note>
  Shops are created and administered from the [Mutasib dashboard](https://mutasib.com/dashboard). The API exposes read and update operations on an existing shop. Each shop is identified by a UUID (e.g. `9c8f4a12-3b7d-4e2a-a1c5-8f6d0e2b9a41`), which you can copy from the shop's settings page in the dashboard.
</Note>
...
response = requests.get(
    "https://api.mutasib.com/api/v1/shops/9c8f4a12-3b7d-4e2a-a1c5-8f6d0e2b9a41",
...
```js JavaScript
const response = await fetch("https://api.mutasib.com/api/v1/shops/9c8f4a12-3b7d-4e2a-a1c5-8f6d0e2b9a41", {
  headers: { Authorization: "Bearer YOUR_API_TOKEN" },
});
...
<ResponseField name="id" type="string">
  Unique shop UUID. Find yours in the [Mutasib dashboard](https://mutasib.com/dashboard) under each shop's settings.
</ResponseField>
